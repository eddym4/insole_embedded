...
uint32_t ble_isd_init(ble_hrs_t * p_hrs, const ble_hrs_init_t * p_hrs_init)
{
    uint32_t   err_code;
    ble_uuid_t ble_uuid;
...
uint32_t ble_isd_pressure_measurement_send(ble_isd_t * p_isd, uint8_t pressure)
{
    uint32_t err_code;
    ...
    if (p_hrs->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        hvx_params.p_data = &pressure;

        err_code = sd_ble_gatts_hvx(p_isd->conn_handle, &hvx_params);
