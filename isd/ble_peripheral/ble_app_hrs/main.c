...
static ble_isd_t                             m_isd;                                     /**< Structure used to identify the in sole device service. */
...
static app_timer_id_t                        m_pressure_timer_id;                     /**< Heart rate measurement timer. */
...
static void pressure_meas_timeout_handler(void * p_context)
{
    ...
    err_code = ble_isd_pressure_measurement_send(&m_isd, pressure);
    if ((err_code != NRF_SUCCESS) &&
        (err_code != NRF_ERROR_INVALID_STATE) &&
        (err_code != BLE_ERROR_NO_TX_BUFFERS) &&
        (err_code != BLE_ERROR_GATTS_SYS_ATTR_MISSING)
...
static void services_init(void)
{
    ...
    ble_hrs_init_t hrs_init;
	  ble_isd_init_t isd_init;
    ble_bas_init_t bas_init;
    ble_dis_init_t dis_init;
    uint8_t        body_sensor_location;
	
	    // Initialize In Sole Service.

    memset(&isd_init, 0, sizeof(isd_init));

    isd_init.evt_handler                 = NULL;
    isd_init.is_sensor_contact_supported = true;

    // Here the sec level for the Heart Rate Service can be changed/increased.
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&isd_init.isd_hrm_attr_md.cccd_write_perm);
    BLE_GAP_CONN_SEC_MODE_SET_NO_ACCESS(&isd_init.isd_hrm_attr_md.read_perm);
    BLE_GAP_CONN_SEC_MODE_SET_NO_ACCESS(&isd_init.isd_hrm_attr_md.write_perm);

    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&isd_init.isd_bsl_attr_md.read_perm);
    BLE_GAP_CONN_SEC_MODE_SET_NO_ACCESS(&isd_init.isd_bsl_attr_md.write_perm);

    err_code = ble_isd_init(&m_isd, &isd_init);
    APP_ERROR_CHECK(err_code);
